import { Box, Container, TextField, Typography, Button, Divider, Input } from '@mui/material';
...
import { DataContext, IContext } from './../../context/DataProvider';
interface ICart {
  attributes: any;
  id: string;
  image: string;
...
export default function AvoDetail({ product }: any) {
  ...
  const findProduct = () => {
    const newCart = cart.map(({ attributes , id, image, name, price, sku, quantity = 0}: ICart) => {
      if (sku === product.sku) {
        return {
          attributes,
          id,
    ...
    <>
      ...
      <Container maxWidth="md" sx={{ marginTop: 6 }}>
        <Box display="flex" alignItems="center" justifyContent="space-evenly">
          ...
          <Box>
            <Typography color="#000000" sx={{ fontSize: 24, fontWeight: 500, mb: 1 }}>
              {product.name}
            </Typography>
            <Typography mt={1}>$ {product?.price}</Typography>
            <Typography
              fontSize={10}
              sx={{ background: '#f2f2f2', borderRadius: 1, p: 0.5, width: 100 }}
            >
              SKU: {product.sku}
            </Typography>
            <Box display="flex" alignItems="center" mt={2}>
              <TextField value={value} type="number" autoComplete="off" onChange={handleChange} sx={{height: 55, mr:2}}/>
              <Button
                onClick={addAvocado}
                color="secondary"
                variant="contained"
        ...
        <Box>
          <Typography sx={{ fontSize: 20, fontWeight: 500, mb: 1 }}>About this avocado</Typography>
